...
from src.entities import AsanaTaskRequest, ReportData
...
async def report_completed_contractor_tasks(
    ...
    async with AsyncAsanaClient(
        ...
        contractor_project = await client.projects.get_project(contractor_project_gid)
        asana_project_name = contractor_project.get('name')

        notes_data = [(t.name, t.permalink_url) for t in contractor_tasks]
        ...
        logs.info('Created main task %s', main_task.gid)
